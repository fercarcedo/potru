...
export function initTour({ svg, TR, openNode }: TourDeps) {
  ...
  function highlight(ids: string[]){
    ...
    if(!reduced)ids.forEach(id=>{
      ...
      svgEl('animateMotion',{dur:'3.4s',repeatCount:'indefinite',path:path.getAttribute('d')!},c);
      pulses.push(c);
    });
  }
  function show(i: number){
    idx=i;const s=TOUR_STOPS[i]!;
    panel.classList.add('open');
    tpStep.textContent=`Paseo por la red · parada ${i+1} de ${TOUR_STOPS.length}`;
    tpTitle.textContent=s.title;tpText.textContent=s.text;
    ...
    setVB(s.viewBox);highlight(s.trunks);
  ...
  tpNext.onclick=()=>{if(idx===TOUR_STOPS.length-1)exitTour();else show(idx+1)};
