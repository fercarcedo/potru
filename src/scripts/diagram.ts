...
export function initDiagram() {
  ...
  function buildDiagram() {
    ...
    el(
      ...
      gOnt,
    ).textContent = 'domicilio del usuario';
    ...
    el(
      'text',
      { x: 300, y: Y + 48, fill: mut, 'font-size': 10, 'text-anchor': 'middle' },
      gS16,
    ).textContent = 'splitter 1:16';
    ...
    el(
      'text',
      { x: 437, y: Y + 42, fill: mut, 'font-size': 10, 'text-anchor': 'middle' },
      gS4,
    ).textContent = 'splitter 1:4';
    el('text', {
      x: 368,
      y: Y - 38,
      fill: passive,
    ...
    el(
      ...
      gOlt,
    ).textContent = 'NODO · OLT';
    el(
      'text',
      { x: 655, y: Y + 80, fill: mut, 'font-size': 9.5, 'text-anchor': 'middle' },
      gOlt,
    ).textContent = mode === 'gpon' ? 'Alcatel 7342 / Ericsson BLM' : 'OLT dual GPON/XGS-PON';
    el(
      'text',
      { x: 655, y: Y + 95, fill: c, 'font-size': 9, 'text-anchor': 'middle' },
      gOlt,
    ).textContent = mode === 'gpon' ? '×20 nodos · enlaces 1 Gbps' : '×20 nodos · ≥10 Gbps';
    ...
    el(
      'text',
      { x: 808, y: Y + 4, fill: txt, 'font-size': 10.5, 'text-anchor': 'middle' },
      gT,
    ).textContent = 'TRONCAL';
    el(
      'text',
      { x: 808, y: Y + 40, fill: mut, 'font-size': 9.5, 'text-anchor': 'middle' },
      gT,
    ).textContent = mode === 'gpon' ? 'propia / alquilada + CWDM' : 'directos + CWDM / nuevos';
    ...
    if (mode === 'gpon') {
      ...
      el(
        ...
        gP,
      ).textContent = '7750SR-7 · 100%';
      el(
        'text',
        { x: 1060, y: Y + 26, fill: mut, 'font-size': 9, 'text-anchor': 'middle' },
        gP,
      ).textContent = 'sin capacidad de';
      el(
        'text',
        { x: 1060, y: Y + 38, fill: mut, 'font-size': 9, 'text-anchor': 'middle' },
        gP,
      ).textContent = 'ampliación';
    } else {
      el(
        'rect',
        {
      ...
      el(
        'text',
        { x: 1060, y: Y + 42, fill: c, 'font-size': 8.5, 'text-anchor': 'middle' },
        gP,
      ).textContent = '+ enrutador en MIERES';
    }
    el(
      'text',
      { x: 1060, y: Y + 82, fill: mut, 'font-size': 9.5, 'text-anchor': 'middle' },
      gP,
    ).textContent = 'jaulas de operadores';
    ...
    el(
      'text',
      { x: 735, y: 58, fill: txt, 'font-size': 11, 'text-anchor': 'middle', 'font-weight': 600 },
      gO,
    ).textContent = mode === 'gpon' ? 'OSS · 6 gestores + NetCool' : 'OSS · gestión integral única';
    el(
      'text',
      { x: 735, y: 74, fill: mut, 'font-size': 9, 'text-anchor': 'middle' },
      gO,
    ).textContent =
      mode === 'gpon'
        ? 'AMS · Entriview · SAM · TNM · GENView · Cimplicity'
        : 'nuevos gestores integrados con OSS de GIT (Fase 1)';
    el('line', {
      x1: 655,
      y1: 84,
      x2: 655,
    ...
    el('text', {
      ...
      mode === 'gpon'
        ? '── RED EN PRODUCCIÓN DESDE 2004 ──'
        : '── RED RENOVADA · MIGRACIÓN SIN CORTE HASTA CONFORMIDAD ──';
  ...
  function setMode(m: 'gpon' | 'xgs') {
    mode = m;
    bG.className = m === 'gpon' ? 'active-gpon' : '';
    bX.className = m === 'xgs' ? 'active-xgs' : '';
    bG.setAttribute('aria-selected', String(m === 'gpon'));
    bX.setAttribute('aria-selected', String(m === 'xgs'));
    buildDiagram();
    infoBox.innerHTML =
      m === 'gpon'
  ...
  bG.onclick = () => setMode('gpon');
  bX.onclick = () => setMode('xgs');
}
